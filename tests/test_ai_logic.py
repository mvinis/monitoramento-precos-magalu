...
# Criação de uma "Fixture" para carregar a IA apenas uma vez para todos os testes
...
def ia():
    """Inicializa o classificador de IA uma única vez para o módulo de testes."""
    return ProductClassifier()

@pytest.fixture
def contexto_padrao():
    """Gera um dicionário de contexto padrão para os testes do pipeline."""
    return {
...
def test_gamepad_nao_deve_ser_smartphone(ia, contexto_padrao):
    ...
    """
    Verifica se acessórios de jogos (Gamepad) são classificados corretamente como 'Console'
    e não são confundidos com o aparelho 'Smartphone'.
    """
    ...
    # O ASSERT: Espera que NÃO contenha "Smartphone"
...
def test_samsung_b310e_deve_ser_celular_basico(ia, contexto_padrao):
    ...
    """
    Valida a identificação de aparelhos antigos ou de baixo custo como 'Celular Básico',
    garantindo que a hierarquia de hardware funcione para modelos sem Android/iOS.
    """
...
def test_suporte_garra_nao_deve_ser_carregador(ia, contexto_padrao):
    ...
    """
    Verifica a prioridade de acessórios estruturais, garantindo que um suporte de moto
    seja categorizado como 'Suporte', mesmo possuindo função secundária de carregamento.
    """
